...
class FuncApp(Flask):
    def __init__(self, name, loglevel=logging.DEBUG):
        ...
        @self.route('/newChat', methods=['GET', 'POST', 'PUT', 'HEAD', 'OPTIONS', 'DELETE'])
        def new_chat():
            request_data = request.json
            self.logger.info(f"new lichat feishu request_body:{request_data}")
        ...
        @self.route('/contextChat', methods=['GET', 'POST', 'PUT', 'HEAD', 'OPTIONS', 'DELETE'])
        def context_chat():
            request_data = request.json
            self.logger.info(f"context lichat feishu request_body:{request_data}")
            ...
            if input_data.get("user_type") != 1:
                res = "暂不支持您使用LiChat"
                self.logger.info(f"not employee, feishu_chat_id:{feishu_chat_id}, request_data:{request_data}")
            ...
            context_lichat_res = get_lichat_res(input_data.get("username"), input_data.get("mobile"),
            ...
            if context_lichat_res:
            #     send_lichat_res_message(input_data.get("env"), feishu_chat_id, input_data.get("chat_type"), context_lichat_res.get("content"),
            #                      input_data.get("username"), loading_message_id)
                return {"code": 200, "data": context_lichat_res}
            else:
                return {"code": 400, "data": "lichat error"}
        ...
        def get_lichat_res(username, mobile, content, env, feishu_chat_id, feishu_message_id, chat_type, loading_message_id):
            ...
            LiChat_url = "https://chat-test.chehejia.com/backend-api/v1/conversation/bot"
            ...
            LiChat_headers = {
                ...
                "X-CHJ-GWToken": os.getenv("APIHUB_TOKEN") if os.getenv("APIHUB_TOKEN") else "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJ6VWg0ME1MUUZJeFRWblpicWMxYUZVSThNVHpZeVJuMiJ9.6cy8bxYNMIydqyGyYr-4XCwvHRTldo-uFeUt8JzjVtw"
            }
            ...
            try:
                for i in range(3):
                    self.logger.info(f"lichat request time={i+1}, lichat request_body:{data}")
                    ...
                    self.logger.info(f"lichat feishu response: username={username}, message_id={gpt_message_id}, "
        ...
        def get_lichat_res_message_card(env, feishu_chat_id, chat_type, lichat_res_content, username, is_end):
            ...
            if chat_type != "group":
                message_elements = [{
                    "tag": "markdown",
                    "content": lichat_res_content if lichat_res_content else "LLM nothing returned! Please try again later！"
                }, {
                    "tag": "hr",
                }, {
                    "tag": "div",
        ...
        # 获取it用户信息
        def get_it_user_info(search_type: str, search_text: str):
            user_data = []
            if not search_type:
                self.logger.error(f"IT ERROR: search_type is none, search_text={search_text}")
                return user_data
            if not search_text:
                self.logger.error(f"IT ERROR: search_text is none, search_type={search_type}")
                return user_data
            user_cache_key = search_type + "-" + search_text
            user_data_str = REDIS.get(user_cache_key)
            user_data = json.loads(user_data_str) if user_data_str else []
            if user_data:
                return user_data
            # 缓存里没有用户信息时，从it获取
            access_token = REDIS.get("it_user_token")
            if not access_token:
                # 获取it token
                token_url = "https://coa-api.it.lixiangoa.com/oauth/token"
                token_params = {
                    "grant_type": "client_credentials",
                    "client_id": 14,
                    "client_secret": "NUechDMlq6Dbjct2QqQXx2A6ujAWgWFgsv61U6LA",
                    "scope": "*"
                }
                token_headers = {
                    "Content-Type": "application/json"
                ...
                try:
                    token_res = requests.post(token_url, data=json.dumps(token_params), headers=token_headers).json()
                    access_token = token_res.get("access_token")
                    expires_time = token_res.get("expires_in")
                    if access_token and expires_time:
                        REDIS.set("it_user_token", access_token, expires_time / 2)
                except Exception as e:
                    self.logger.error(f"IT ERROR: get token error, search_type={search_type}, search_text={search_text}")
                    return user_data

            it_user_url = f"https://coa-api.it.lixiangoa.com/api/info/search_in_all_user?search_type={search_type}&search_text={search_text}"
            user_headers = {
                "Authorization": f"Bearer {access_token}"
            }
            try:
                it_user_info = requests.get(it_user_url, headers=user_headers).json()
                if it_user_info.get("data"):
                    user_info_set_str = json.dumps(it_user_info.get("data"))
                    REDIS.set(user_cache_key, user_info_set_str)
                    user_data = it_user_info.get("data")
                else:
                    self.logger.error(f"IT ERROR: user info is none, search_type={search_type}, search_text={search_text}")
            except Exception as e:
                self.logger.error(f"IT ERROR: get user info error, search_type={search_type}, search_text={search_text}")
            return user_data

        # 获取用户手机号
        def get_mobile(username):
            user_info = get_it_user_info("ldap", username)
            if user_info:
                return user_info[0].get("mobile")
            return ""
...
# app.logger.info(f"Starting gevent based server")
